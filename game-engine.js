...
const GameEngine = {
    ...
    canUnlockTrial5() {
        ...
        const now = new Date(); const aptDateStr = this.state.appointmentTime.replace(/\//g, '-'); 
        const aptTimeParts = aptDateStr.split(' '); const dateStr = aptTimeParts[0];
    ...
    completeTrial(event, trialNum) {
        ...
        this.save(); this.updateButtonStyles(); 
        let payload = {};
    ...
    handleFileUpload(inputElement, chkId) {
        ...
        let typeLabel = "其他證明";
        if(chkId === 'chk-t3-1') typeLabel = "勞工體檢"; else if(chkId === 'chk-t3-2') typeLabel = "從業體檢"; else if(chkId === 'chk-t3-3') typeLabel = "存摺"; else if(chkId === 'chk-t3-opt1') typeLabel = "健保轉出"; else if(chkId === 'chk-t3-opt2') typeLabel = "離職證明";
        ...
        reader.onload = (e) => {
            ...
            const payload = { fileUploads: [{ base64: base64Data, mimeType: file.type, fileName: file.name, typeLabel: typeLabel }] };
            if (this.state.sysId) {
                payload.id = this.state.sysId;
                ...
                setTimeout(() => { if(statusSpan) { statusSpan.innerText = "✅ 已上傳 (單機測試)"; statusSpan.classList.add('success'); } const chkBox = document.getElementById(chkId); if (chkBox) chkBox.checked = true; }, 1500);
            }
        };
    ...
    showFinalAchievement(withFirework = true) {
        ...
        const renderModal = () => {
            ...
            document.body.appendChild(modal); void modal.offsetWidth; modal.classList.add('active');
        };
        if (withFirework) {
            const fw = document.createElement('div'); fw.id = 'firework-overlay';
            fw.innerHTML = `<div class="css-firework fw-1"></div><div class="css-firework fw-2"></div><div class="css-firework fw-3"></div><div class="firework-text">入職試煉圓滿達成<br>歡迎正式踏入我們的行列</div>`;
            document.body.appendChild(fw); void fw.offsetWidth; fw.classList.add('active');
            setTimeout(() => { fw.classList.remove('active'); setTimeout(() => { fw.remove(); renderModal(); }, 500); }, 3000); 
        } else { renderModal(); }
    },
        ...
        trials.forEach(n => {
            ...
            if (btn) {
                if (this.state.currentTrial >= n) {
                    btn.disabled = true; btn.innerText = lockedTexts[n];
                    if (detailsBlock) {
                        const inputs = detailsBlock.querySelectorAll('input');
                        inputs.forEach(input => { input.disabled = true; if(input.type === 'checkbox' || input.type === 'radio' || input.type === 'file' || input.type === 'text') { input.style.opacity = "0.5"; input.style.cursor = "not-allowed"; } });
                        const uploadBtns = detailsBlock.querySelectorAll('.file-upload-btn');
                        uploadBtns.forEach(uBtn => { uBtn.style.opacity = "0.5"; uBtn.style.cursor = "not-allowed"; uBtn.style.pointerEvents = "none"; });
                    }
                    if (n === 6) { btn.disabled = false; btn.style.cursor = "pointer"; btn.onclick = () => { this.showFinalAchievement(false); }; }
                }
            }
            if (detailsBlock) {
                if (n === 1) { detailsBlock.classList.remove('locked-details'); } 
                else { if (this.state.currentTrial >= n - 1) { detailsBlock.classList.remove('locked-details'); } else { detailsBlock.classList.add('locked-details'); detailsBlock.removeAttribute('open'); } }
            }
        });
        if(this.state.achievements.includes('t5_score_1')) { const chk = document.getElementById('chk-t5-1'); if (chk && this.state.currentTrial < 5) chk.checked = true; }
        if(this.state.achievements.includes('t5_score_2')) { const chk = document.getElementById('chk-t5-2'); if (chk && this.state.currentTrial < 5) chk.checked = true; }
        const have = document.getElementById('chk-bank-have'); const process = document.getElementById('chk-bank-process'); const done = document.getElementById('chk-bank-done'); const bDate = document.getElementById('input-bank-date'); const bBtn = document.getElementById('btn-lock-bank');
        if (have && process && done && this.state.currentTrial < 6) {
            have.disabled = false; process.disabled = false; done.disabled = false; if(!this.state.bankDateLocked) { bDate.disabled = false; bBtn.disabled = false; }
            if (this.state.bankStatus === 'have') { have.checked = true; process.disabled = true; done.disabled = true; bDate.disabled = true; bBtn.disabled = true; } 
            else if (this.state.bankStatus === 'process') { process.checked = true; have.disabled = true; done.disabled = true; } 
            else if (this.state.bankStatus === 'done') { done.checked = true; have.disabled = true; process.disabled = true; bDate.disabled = true; bBtn.disabled = true; }
        }
    }
};
